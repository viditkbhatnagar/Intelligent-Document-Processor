...
exports.TemplatePopulationController = void 0;
const template_population_service_1 = require("../services/template-population.service");
class TemplatePopulationController {
    constructor() {
        this.templatePopulationService = new template_population_service_1.TemplatePopulationService();
    }
    async populateFromInvoice(req, res) {
        try {
    ...
    async previewPopulation(req, res) {
        try {
            // Demo mode - use dummy user if no authentication
            if (!req.user) {
                req.user = { userId: 'demo-user' };
            ...
            const preview = await this.templatePopulationService.previewTemplatePopulation(extractedFields, templateId);
            res.json({
    ...
    async enhanceFields(req, res) {
        try {
            ...
            const enhancedFields = await this.templatePopulationService.enhanceFieldExtraction(documentId, additionalFields);
            res.json({
    ...
    async downloadPopulatedDocument(req, res) {
        try {
            // Demo mode - use dummy user if no authentication
            if (!req.user) {
                req.user = { userId: 'demo-user' };
...
exports.TemplatePopulationController = TemplatePopulationController;
//# sourceMappingURL=template-population.controller.js.map