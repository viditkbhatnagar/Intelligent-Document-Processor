...
exports.POGenerationService = void 0;
...
class POGenerationService {
     ...
     */
    async generatePOAndPFI(request) {
        try {
            ...
            const processedQuotation = await Document_1.ProcessedDocumentModel.findById(quotationDoc.documentId).exec();
            ...
            const po = await this.generatePO(transaction, processedQuotation, extractedData, company, supplier, request);
            // Generate PFI in AED
            const pfi = await this.generatePFI(transaction, extractedData, company, customer, request);
     ...
     */
    extractItemsFromQuotation(fields) {
        const items = [];
        let totalAmount = 0;
        let currency = 'USD';
        ...
        if (currencyField) {
            ...
            if (value.includes('usd') || value.includes('$'))
                ...
                currency = 'EUR';
            else if (value.includes('aed') || value.includes('dh'))
                currency = 'AED';
        }
        // Extract items (this is a simplified version - in production you'd want more sophisticated parsing)
        const itemFields = fields.filter(f => f.key.toLowerCase().includes('item') ||
            f.key.toLowerCase().includes('product') ||
            f.key.toLowerCase().includes('description'));
        const quantityFields = fields.filter(f => f.key.toLowerCase().includes('quantity') ||
            f.key.toLowerCase().includes('qty'));
        const priceFields = fields.filter(f => f.key.toLowerCase().includes('price') ||
        ...
        if (items.length === 0) {
            const totalField = fields.find(f => f.key.toLowerCase().includes('total') && f.type === 'currency');
            if (totalField) {
     ...
     */
    async generatePO(transaction, quotation, extractedData, company, supplier, request) {
...
ITEMS:
${extractedData.items.map((item, index) => `${index + 1}. ${item.itemName}
     Description: ${item.description}
     Quantity: ${item.quantity} ${item.unit}
     Unit Price: ${extractedData.currency} ${item.unitPrice.toFixed(2)}
     Total: ${extractedData.currency} ${item.totalPrice.toFixed(2)}`).join('\n\n')}
...
Date: ${new Date().toDateString()}
     ...
     */
    async generatePFI(transaction, extractedData, company, customer, request) {
        ...
        const itemsAED = extractedData.items.map((item) => ({
            ...item,
...
Date: ${new Date().toDateString()}
     ...
     */
    async saveToPurchaseHistory(transaction, extractedData, request) {
        try {
            const exchangeRate = this.getExchangeRate(extractedData.currency);
            const purchaseHistory = new PurchaseHistory_1.PurchaseHistoryModel({
...
exports.POGenerationService = POGenerationService;
//# sourceMappingURL=po-generation.service.js.map