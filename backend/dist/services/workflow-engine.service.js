...
class WorkflowEngine {
     ...
     */
    async processDocumentWorkflow(document) {
        try {
     ...
     */
    async findRelatedTransaction(document) {
     ...
     */
    areDocumentsRelated(document, transaction, currentEntities) {
    ...
    extractCompanyName(entity) {
        if (!entity)
            return null;
        return entity.name || entity.companyName || null;
    }
    extractFieldValue(fields, keys) {
        if (!fields)
            return null;
     ...
     */
    async createNewTransaction(document) {
        ...
        await Document_1.ProcessedDocumentModel.findByIdAndUpdate(document._id, {
     ...
     */
    async updateExistingTransaction(transaction, document) {
        // Add document to transaction
        const updatedTransaction = await BusinessTransaction_1.BusinessTransactionModel.findByIdAndUpdate(transaction._id, {
            $push: {
                documents: {
                    documentId: document._id,
                    documentType: document.documentType,
        ...
        if (newStep) {
            const suggestions = await this.generateWorkflowSuggestions(newStep, await this.getTransactionDocumentsInternal(updatedTransaction));
            await BusinessTransaction_1.BusinessTransactionModel.findByIdAndUpdate(transaction._id, {
     ...
     */
    determineInitialWorkflowStep(documentType) {
        switch (documentType) {
            case 'quotation':
     ...
     */
    async shouldAdvanceWorkflow(transaction, newDocument) {
        const currentStep = transaction.currentStep;
        const documentTypes = transaction.documents.map((d) => d.documentType);
        ...
        switch (currentStep.stepId) {
            ...
            case 'invoice_received':
                ...
                const hasInvoice = documentTypes.some((type) => ['commercial_invoice', 'tax_invoice', 'invoice'].includes(type));
     ...
     */
    async generateWorkflowSuggestions(currentStep, documents) {
        const suggestions = [];
     ...
     */
    async extractEntitiesFromDocument(document) {
     ...
     */
    async getTransactionDocumentsInternal(transaction) {
        const documentIds = transaction.documents.map((d) => d.documentId);
        return await Document_1.ProcessedDocumentModel.find({
            _id: { $in: documentIds }
        });
    }
    /**
    ...
    async getUserTransactions(userId) {
        return await BusinessTransaction_1.BusinessTransactionModel.find({ userId }).sort({ updatedDate: -1 });
    }
    /**
     * Get transaction by ID
     */
    async getTransaction(transactionId, userId) {
        return await BusinessTransaction_1.BusinessTransactionModel.findOne({ transactionId, userId });
    }
    /**
     * Update transaction manually
     */
    async updateTransactionStatus(transactionId, userId, status, notes) {
        const transaction = await BusinessTransaction_1.BusinessTransactionModel.findOne({ transactionId, userId });
        if (!transaction) {
            return null;
        }
        const newStep = Object.values(this.workflowSteps).find(step => this.getStatusFromStep(step.stepId) === status);
        if (!newStep) {
        ...
        return await BusinessTransaction_1.BusinessTransactionModel.findByIdAndUpdate(transaction._id, {
            status,
            currentStep: newStep,
            nextSuggestedActions: suggestions,
            updatedDate: new Date()
