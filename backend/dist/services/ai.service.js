...
class AIService {
    ...
    async mapFieldsToTemplate(extractedFields, template) {
        try {
    ...
    async generatePopulatedTemplate(extractedFields, template) {
        try {
    ...
    findBestFieldMatch(extractedFields, templateField) {
    ...
    buildMappingPrompt(extractedFields, template) {
        const templateFieldsStr = template.fields.map(f => `- ${f.key} (${f.label}): type=${f.type}, hints=[${f.mappingHints.join(', ')}]`).join('\n');
        const extractedFieldsStr = extractedFields.map(f => `- ${f.key}: "${f.value}" (confidence: ${f.confidence}, type: ${f.type})`).join('\n');
        return `
    ...
    parseMappingResponse(response, template) {
        try {
            const cleanResponse = response.trim();
            const jsonStart = cleanResponse.indexOf('{');
            const jsonEnd = cleanResponse.lastIndexOf('}') + 1;
            ...
            const jsonStr = cleanResponse.substring(jsonStart, jsonEnd);
            const parsed = JSON.parse(jsonStr);
            const mappedFields = (parsed.mappedFields || []).map((field) => ({
     ...
     */
    async extractDocumentEntities(extractedFields, documentType) {
        try {
     ...
     */
    async areDocumentsRelated(newDocument, existingDocumentIds) {
        try {
