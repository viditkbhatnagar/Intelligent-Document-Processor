...
export interface CollectionDocumentsRequest {
  ...
  transportDocumentType: string; // mime type
...
export class CollectionDocumentsService {
   ...
   */
  async generateCollectionDocuments(request: CollectionDocumentsRequest): Promise<{
    ...
    try {
      ...
      const transportDocData = await this.processTransportDocument(
        request.transportDocumentFile,
        request.transportDocumentType,
      ...
      const coveringLetter = this.generateCoveringLetter(
        transaction,
        company,
        customer,
        request,
      ...
      const billOfExchange = this.generateBillOfExchange(
        transaction,
        company,
        customer,
        request,
      ...
      await this.storeTransportDocument(
        request,
        transportDocData,
   ...
   * Process transport document to extract date and number
   */
  private async processTransportDocument(
    fileBuffer: Buffer,
    mimeType: string,
    shipmentMethod: string
  ): Promise<TransportDocumentData> {
    try {
  ...
  private generateCoveringLetter(
    ...
    transportDoc: TransportDocumentData
  ): CollectionDocument {
...
${bankDetails.footer || ''}
   ...
   */
  private async storeTransportDocument(
    request: CollectionDocumentsRequest,
    transportDocData: TransportDocumentData,
    transactionId: string
  ): Promise<void> {
    try {
      // Store transport document file (simplified - in production use GridFS)
      const transportDocument = new ProcessedDocumentModel({
   ...
   */
  private getBankDetails(bankName: string) {
    const banks = {
      'ADIB': {
    ...
    return banks[bankName as keyof typeof banks] || banks['ADIB'];
  }
  ...
  private numberToWords(amount: number): string {
    ...
    if (rounded < 1000000) return `${Math.round(rounded/1000)} Thousand`;
