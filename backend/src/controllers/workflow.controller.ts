...
export class WorkflowController {
   ...
   */
  async getUserTransactions(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
  ...
  async updateTransactionStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const transaction = await this.workflowEngine.updateTransactionStatus(
        transactionId, 
        req.user.userId, 
        status, 
        notes
  ...
  async getDashboard(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const statusBreakdown = transactions.reduce((acc: any, transaction) => {
        acc[transaction.status] = (acc[transaction.status] || 0) + 1;
        return acc;
      }, {});

      // Get high priority actions across all transactions
      const highPriorityActions = transactions.flatMap(t => 
        (t.nextSuggestedActions || []).filter((action: any) => action.priority === 'high')
      );
   ...
   */
  async reevaluateWorkflows(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      // Demo mode - use dummy user if no authentication
      if (!req.user) {
        req.user = { userId: 'demo-user' };
  ...
  async getAnalytics(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const averageProcessingTimes = Object.keys(processingTimes).reduce((acc: any, status) => {
      ...
      const documentTypeDistribution = transactions.reduce((acc: any, transaction) => {
